...
function loadTimeline(teacherId, semesterId) {
    ...
    $.getJSON("http://localhost:8080/api/employee/getSchedule/" + teacherId + "/" + semesterId, function (response) {
        ...
        console.log(listCourseOffering);
        let out = '<tr>'
            + '<th id="r1">1</th>'
            + '<td id="r1t2"></td>'
            + '<td id="r1t3"></td>'
        ...
        for (let i = 0; i < listCourseOffering.length; i++)
        {
            let course = listCourseOffering[i];
            let startShift = parseInt(course.startShift);
            let endShift = parseInt(course.endShift);
            for (let j = startShift; j <= endShift; j++){
                let id = "r" + j + "t" + listCourseOffering[i].dayOfWeek;
                if (j === startShift) {
                    $("#" + id).html(listCourseOffering[i].lessonName + "<br/>" + listCourseOffering[i].roomName);
